...
for i in range(n_instants):
    total_hour = base_hour + i * dt_hours

    day = base_day + total_hour // 24
    hour = total_hour % 24

    INSTANCES.append(
        (float(i), base_year, day, hour, base_minute, base_second)
    )
...
def main() -> None:
    ...
    for idx, (t, year, day, hour, minute, second) in enumerate(INSTANCES):
        field_file = OUT / f"T01_field_{idx:03d}.vtk"
        seed_file = OUT / f"T01_SM_seeds_{idx:03d}.vtk"
    ...
    write_pvd(OUT / "T01_field_timeseries.pvd", field_files)
    write_pvd(OUT / "T01_SM_seeds_timeseries.pvd", seed_files)

    print("\nGenerated:")
    print(OUT / "T01_field_timeseries.pvd")
    print(OUT / "T01_SM_seeds_timeseries.pvd")
